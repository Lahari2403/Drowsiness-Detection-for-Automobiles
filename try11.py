...
ALERT_THRESHOLD = 3  # Alert after 3 detections of either drowsiness or yawning
...
while True:
    ...
    for face in faces:
        ...
        # Send Email Alert
        if drowsiness_count >= ALERT_THRESHOLD:
            send_email_alert(
                "Drowsiness Alert",
                f"Drowsiness detected {drowsiness_count} times. Please stay alert."
            )
            drowsiness_count = 0  # Reset count after email

        if yawn_count >= ALERT_THRESHOLD:
            send_email_alert(
                "Yawning Alert",
                f"Yawning detected {yawn_count} times. Please stay alert."
            )
            yawn_count = 0  # Reset count after email
