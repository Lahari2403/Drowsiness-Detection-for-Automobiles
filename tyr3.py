...
def send_alert_message():
    api_url = "https://example.com/send_alert"  # Replace with actual API endpoint
    data = {
        "message": "Drowsiness detected 3 times. Immediate action recommended.",
        "recipient": "your_recipient_number_or_id"  # Update with your recipient details
    }
...
predictor = dlib.shape_predictor("C:\\Users\\Dell\\OneDrive\\Desktop\\vigileye\\vigil\\Model\\shape_predictor_68_face_landmarks (1).dat")
...
while True:
    ...
    for face in faces:
        ...
        # Detect Drowsiness
       # Inside your main loop where drowsiness detection occurs
if ear < EAR_THRESHOLD:
    ...
    if frame_count >= CONSEC_FRAMES:
        ...
        # Check if drowsiness count reaches threshold
        if drowsiness_count == 3:
            send_alert_message()  # Send alert message
            drowsiness_count = 0  # Reset after sending message
...
plt.plot(range(len(frame_count)), frame_count, label="Frame Count")
plt.xlabel("Time")
plt.ylabel("Detection Events")
plt.title("Drowsiness Detection Accuracy")
plt.legend()
plt.show()
